...
STL_FILE = os.path.expanduser("~/Desktop/Top55mmTrackball v2.stl")
...
wire_t2  = Wire.make_polygon([Vector(327, y, z) for y, z in tri_pts2], close=True)
solid_t2 = Solid.extrude(Face(wire_t2), Vector(-40, 0, 0))

result = result.fuse(solid_t2)
try:    result = result.solids()[0]
except: pass
print(f"✅ Triangle | Volume: {result.volume:.4e}")
print(f"✅ Triangle | Volume: {result.volume:.4e}")

# Conical hole at X=288 face, drill in -X direction
CONE_TOP_R = 46 / 2   # 14mm
CONE_BOT_R = 5  / 2   # 2.5mm
CONE_DEPTH = 36

cx, cy, cz = 310, 52, -5.536

cone = Solid.make_cone(CONE_TOP_R, CONE_BOT_R, CONE_DEPTH)
cone = cone.rotate(Axis.Y, 90)   # +X direction se andar
cone = cone.moved(Location(Vector(cx - CONE_DEPTH, cy, cz)))
...
print(f"✅ Cone | Volume: {result.volume:.4e}")
# 3x pattern around revolve axis
CONE_TOP_R = 46 / 2
CONE_BOT_R = 5  / 2
CONE_DEPTH = 36

base_cx, base_cy, base_cz = 310, 52, -5.536

rev_axis_dir = Vector(0, math.cos(-ang_rad), math.sin(-ang_rad))
rev_axis_pt  = Vector(0, 0, 0)

for pa in [0, 120, 240]:
    cone = Solid.make_cone(CONE_TOP_R, CONE_BOT_R, CONE_DEPTH)
    cone = cone.rotate(Axis.Y, 90)
    cone = cone.moved(Location(Vector(
        base_cx - CONE_DEPTH,
        base_cy,
        base_cz
    )))
    # Rotate around revolve axis
    cone = cone.rotate(
        Axis(rev_axis_pt, rev_axis_dir),
        pa
    )
    result = result.cut(cone)
    try:    result = result.solids()[0]

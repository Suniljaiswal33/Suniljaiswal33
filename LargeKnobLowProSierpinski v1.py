...
STL_FILE = os.path.expanduser("~/Desktop/LargeKnobLowProSierpinski v1.stl")
...
# closed boundary on top plane
wire_c = Wire.make_polygon(
    [Vector(x,y,z_top) for x,y in cut_pts],
    close=True
)
...
cutter = Solid.extrude(
    Face(wire_c),
    Vector(0,0,-15)   # use -Y based on your model direction
)
...
print(f"✅ 15 mm Triangle Cut | Volume: {result.volume:.4e}")

print("✅ STL saved:", STL_FILE)
print(f"Volume: {result.volume:.4e}")
# =========================
# 15 mm TRIANGLE MATERIAL ADD
# =========================
...
# closed boundary on top plane
wire_a = Wire.make_polygon(
    [Vector(x,y,z_top) for x,y in add_pts],
    close=True
)

# 15 mm material add
feature = Solid.extrude(
    Face(wire_a),
    Vector(0,0,-15)   # +Y add direction
)

# fuse to existing part
result = result.fuse(feature)
...
print(f"✅ 15 mm Triangle Add | Volume: {result.volume:.4e}")
# Scale whole model
result = result.scale(0.101)

print(f"✅ Scaled Volume: {result.volume:.4e}")

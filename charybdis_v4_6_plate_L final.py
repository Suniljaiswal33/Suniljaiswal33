...
DXF_FILE = os.path.join(desktop, "charybdis_v4_6_plate_L final.dxf")
STL_FILE = os.path.join(desktop, "charybdis_v4_6_plate_L final.stl")
...
for i, loop in enumerate(slot_profiles):
    try:
        wire = Wire.make_polygon([Vector(x, y, HEIGHT-CUT_DEPTH) for x, y in loop])
        face = Face(wire)
...
for i, (cx, cy) in enumerate(cs_points):
    try:
        cone = Cone(
            bottom_radius=CS_INNER_R,
            top_radius=CS_OUTER_R,
            height=CS_DEPTH
        )
